 ...
 */
function createPin(city, width, height, longUnit, latUnit, hash, error = false) {
...
async function populateMap(src, svg, tooltip, opts) {
  ...
  let cities = await fetchData(src);
  if (opts.cdn) {
    cities = cities.filter((city) => city.Provider.toLowerCase() === opts.cdn);
  }
  // iterate over the cities data to create pins for each city
  cities.forEach((city) => {
    const errored = opts.errored[city.Code];
    const pin = createPin(city, width, height, longUnit, latUnit, opts.hashes[city.Code], errored);
    group.append(pin);
